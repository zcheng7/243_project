...
test_that("Test empirical distributions", {
  ...
  out <- ars(function(x){dbeta(x, shape1 = 2, shape2 = 2)}, n=1000, lb=0, ub=1)
  ...
  out <- ars(function(x){dchisq(x, df = 5)}, n=1000, lb=0)
  pv <- ks.test(out, rchisq(length(out), 6))$p.value
  print("KS test estimate p-value is: ")
  print(pv)
  cat("\n\n")
